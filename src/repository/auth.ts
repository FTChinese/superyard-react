...
import { ApiErrorPayload, ResponseError } from '../http/response-error';
...
export function login(c: CMSCredentials): Promise<CMSPassport> {
  return axios
    .post<CMSPassport, AxiosResponse<CMSPassport>, CMSCredentials>(
      endpoint.login,
      c
    )
    .then((resp) => {
...
export function requestPasswordReset(req: PwResetLetterReq): Promise<boolean> {
  return axios
    .post(endpoint.requestPasswordReset, req)
    .then((resp) => {
...
export function verifyPwToken(token: string): Promise<PasswordResetVerified> {
  return axios
    .get<never, AxiosResponse<PasswordResetVerified>>(
      endpoint.verifyResetToken(token),
      {
